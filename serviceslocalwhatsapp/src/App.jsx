...
// Picked once at signup. Each type loads its own 5-7 category quick-chips.
...
export default function App() {
  ...
  const [modalVariant, setModalVariant] = useState(null)        // picked duration/variant in the item modal
  const [modalModifiers, setModalModifiers] = useState([])      // checked add-ons in the modal
  ...
  const [formAllergens, setFormAllergens] = useState([])   // SERVICES: "Requirements" — things customer must provide
  const [formDietary, setFormDietary] = useState([])       // SERVICES: "Service Tags" — Mobile, Eco-friendly, Same-day...
  const [formPortion, setFormPortion] = useState('')       // SERVICES: "Coverage" — "Up to 50sqm" / "Single visit"
  const [formStock, setFormStock] = useState('')           // empty = unlimited; number = available slots
  const [formVariants, setFormVariants] = useState([])     // [{id, name, priceDelta}] — duration tiers
  const [formModifiers, setFormModifiers] = useState([])   // [{id, name, priceDelta}] — add-ons
  ...
  // Shared progressive-disclosure block for both add + edit item forms (services-themed)
  ...
  // but different duration/add-on picks become separate cart lines.
  ...
  const addToCart = useCallback((item, qty = 1, variant = null, modifiers = []) => {
    setCart((prev) => {
      ...
      // Display name includes variant suffix (duration tier); modifier list shown below in cart
  ...
  /* --- Vendor type (home / beauty / automotive / professional / general) --- */
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 200 }} onClick={() => setItemModal(null)}>
          ...
          <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 480, margin: '0 auto', overflowY: 'auto', height: '100%' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ margin: '-20px 12px 0', background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', borderRadius: 20, padding: '20px 18px', position: 'relative', border: isCustomAccent ? `1px solid ${accent}40` : '1px solid rgba(255,255,255,0.08)' }}>
              ...
              {/* Requirements panel — surfaces only if requirements present */}
            ...
            {/* Duration picker — required choice if item has duration tiers */}
