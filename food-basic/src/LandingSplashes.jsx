/* ─────────────────────────────────────────────────────────────
   ...
   bgImg, etc.) — so the existing text / colour / size controls
   drive every theme without rewiring.
   ...
   The frame, background image, and onEnter click handler are
   managed by the caller (App.jsx around line 5611). These
   components render content layered above the bg.
   ───────────────────────────────────────────────────────────── */
...
const fontStack = { sans: 'system-ui, -apple-system, "Segoe UI", Roboto, sans-serif' }

/* ── THEME #1 — Classic ─────────────────────────────────────── */
/* Centered logo + name + tag + button. Original demo used red.
   Now: logo bg = accent, headline = white, button = accent.   */
export function SplashClassic ({ landing = {}, accent = '#DC2626', onEnter }) {
  ...
  return clickable(onEnter, (
    <div style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden', fontFamily: fontStack.sans }}>
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center', gap: 16 }}>
        <div style={{ width: 128, height: 128, borderRadius: 64, background: accent, border: '2px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
          <span style={{ fontSize: 48, fontWeight: 900, color: '#fff' }}>{initial}</span>
        </div>
        <h1 style={{ fontSize: 36, fontWeight: 900, color: '#fff', letterSpacing: -0.5, margin: 0, textShadow: '0 2px 8px rgba(0,0,0,0.7)' }}>{shopName}</h1>
...
export function SplashGlassCard ({ landing = {}, accent = '#EF4444', onEnter }) {
  ...
  return clickable(onEnter, (
    <div style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden', color: '#fff', fontFamily: fontStack.sans, padding: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 390, borderRadius: 40, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', padding: 24, boxShadow: `0 25px 50px -12px ${accent}33`, overflow: 'hidden' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 16px', borderRadius: 999, background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.1)', marginBottom: 32 }}>
        ...
        <h1 style={{ fontSize: 44, lineHeight: 1.05, fontWeight: 900, letterSpacing: -1, margin: 0 }}>
          {head1}<br />{head2}
        ...
        <div style={{ marginTop: 28, display: 'flex', flexDirection: 'column', gap: 14 }}>
          <button onClick={onEnter} style={{ height: 56, borderRadius: 16, background: accent, color: '#fff', fontSize: 18, fontWeight: 700, border: 'none', boxShadow: `0 25px 50px -12px ${accent}66`, cursor: 'pointer' }}>{cta}</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginTop: 14 }}>
          {[stat1, stat2, stat3].map((s, i) => (
            <div key={i} style={{ borderRadius: 22, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', padding: 14, backdropFilter: 'blur(12px)' }}>
              <div style={{ fontSize: 20, fontWeight: 900, color: i === 0 ? accent : (i === 1 ? '#FB923C' : '#FACC15') }}>{s.n}</div>
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', marginTop: 6, lineHeight: 1.2 }}>{s.l}</div>
            </div>
          ))}
        </div>
      </div>
...
export function SplashDiscover ({ landing = {}, accent = '#06B6D4', onEnter }) {
  ...
  return clickable(onEnter, (
    <div style={{ position: 'relative', width: '100%', height: '100%', background: '#070707', overflow: 'hidden', color: '#fff', fontFamily: fontStack.sans, padding: '16px 20px' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, paddingTop: 8 }}>
        ...
        <div style={{ position: 'relative', borderRadius: 36, padding: '28px 24px 40px', background: `linear-gradient(135deg, ${accent} 0%, #3B82F6 50%, #9333EA 100%)`, boxShadow: '0 20px 80px rgba(34,211,238,0.35)', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 2, paddingTop: 60 }}>
            ...
            <h1 style={{ fontSize: 52, lineHeight: 0.95, fontWeight: 900, letterSpacing: -1.5, margin: '0 0 20px', maxWidth: 320 }}>
              {head1}<br />{head2}
...
export function SplashFloat ({ landing = {}, accent = '#F97316', onEnter }) {
  ...
  return clickable(onEnter, (
    <div style={{ position: 'relative', width: '100%', height: '100%', background: '#050505', overflow: 'hidden', color: '#fff', fontFamily: fontStack.sans, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 360, textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: 64, lineHeight: 0.88, fontWeight: 900, letterSpacing: -2.5, margin: '0 0 24px' }}>
          {head1}<br />{head2}
...
export function SplashWarmCard ({ landing = {}, accent = '#FACC15', onEnter }) {
  ...
  return clickable(onEnter, (
    <div style={{ position: 'relative', width: '100%', height: '100%', background: '#060606', overflow: 'hidden', color: '#fff', fontFamily: fontStack.sans, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 360, borderRadius: 36, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', backdropFilter: 'blur(24px)', padding: 28, overflow: 'hidden', boxShadow: '0 30px 120px rgba(0,0,0,0.4)' }}>
        ...
        <h1 style={{ fontSize: 58, lineHeight: 0.85, letterSpacing: -2, fontWeight: 900, margin: '0 0 16px' }}>
          {head1}<br />{head2}
          <span style={{ display: 'block', color: accent }}>{head3}</span>
        </h1>
