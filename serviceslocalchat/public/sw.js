/* Services Local Chat — service worker for vendor push notifications */
const VENDOR_INBOX_URL = '/services/chat/?view=vendor-inbox'
...
self.addEventListener('push', (event) => {
  ...
  const tag = data.conversationId ? `conv-${data.conversationId}` : 'serviceslocalchat-order'
  const options = {
    body,
    icon: '/services/chat/favicon.svg',
    badge: '/services/chat/favicon.svg',
    tag,
    renotify: true,
...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil((async () => {
    ...
    for (const client of allClients) {
      try {
        if (client.url.includes('/services/chat/')) {
          await client.focus()
