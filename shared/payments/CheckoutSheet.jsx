 ...
 *
 * After payment succeeds the host app drops the customer into the order
 * chat thread (no separate email receipt — Stripe/PayPal/Xendit send
 * their own where applicable, and the chat thread serves as the
 * customer's permanent record).
 *
      ...
      try {
        ...
        for (let i = 0; i < 10; i++) {
          ...
          if (row && paidValues.includes(row[paymentField])) {
            setPhase('confirmed'); setMsg('Payment confirmed! Opening chat with the vendor…')
