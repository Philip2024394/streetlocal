...
/* ─── Supabase Vendor Service (ServicesLocal module) ─── */
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const vid = localStorage.getItem('serviceslocalemail_vendorId')
    if (!vid || String(vid).startsWith('local')) return

    // Auto-heal: detect and fix broken states
    const errors = []
    const theme = localStorage.getItem('serviceslocalemail_theme')
    const accentColor = localStorage.getItem('serviceslocalemail_accentColor')
    const themeBg = localStorage.getItem('serviceslocalemail_themeBg')

    // Fix invalid accent color
    if (accentColor && !/^#[0-9A-Fa-f]{6}$/.test(accentColor)) {
      localStorage.setItem('serviceslocalemail_accentColor', '#8DC63F')
    ...
    if (theme && theme !== 'custom' && !themeBg) {
      ...
      if (preset) { localStorage.setItem('serviceslocalemail_themeBg', preset.img); errors.push('Missing theme bg restored') }
    }
    // Fix NaN prices in menu
    try {
      const menu = JSON.parse(localStorage.getItem('serviceslocalemail_menu') || '[]')
      let fixed = false
      menu.forEach(item => { if (isNaN(item.price) || item.price < 0) { item.price = 0; fixed = true } })
      if (fixed) { localStorage.setItem('serviceslocalemail_menu', JSON.stringify(menu)); errors.push('NaN prices fixed') }
    ...
    supabase.from('vendor_status').select('force_reset, reset_theme, reset_accent').eq('vendor_id', vid).maybeSingle().then(({ data }) => {
      if (data?.force_reset) {
        ...
        if (preset) {
          localStorage.setItem('serviceslocalemail_theme', newTheme)
          localStorage.setItem('serviceslocalemail_themeBg', preset.img)
          localStorage.setItem('serviceslocalemail_accentColor', newAccent)
    ...
    // Report health
    const menuCount = JSON.parse(localStorage.getItem('serviceslocalemail_menu') || '[]').length
  ...
  useEffect(() => {
    ...
    const storedId = localStorage.getItem('serviceslocalemail_vendorId')
  ...
  const [showLanding, setShowLanding] = useState(() => {
    ...
    const id = new URLSearchParams(window.location.search).get('vendor') || localStorage.getItem('serviceslocalemail_vendorId') || localStorage.getItem('serviceslocalemail_vendor_id')
    return !id
  })
  const [menuItems, setMenuItems] = useState(() => isDemo ? DEMO_MENU : loadJSON('serviceslocalemail_menu', DEMO_MENU))
  ...
  const [isVendor, setIsVendor] = useState(() => {
    ...
    if (urlVendor) {
      localStorage.setItem('serviceslocalemail_vendorId', urlVendor)
      localStorage.setItem('serviceslocalemail_vendor_id', urlVendor)
      if (urlCity) localStorage.setItem('serviceslocalemail_shopCity', urlCity)
      if (urlCountry) localStorage.setItem('serviceslocalemail_shopCountry', urlCountry)
      // Auto-set delivery rates for new vendors
      const countryCode = params.get('cc')
      if (countryCode && !localStorage.getItem('serviceslocalemail_delRatesSet')) {
        const rates = getDeliveryDefaults(countryCode, urlCity)
        localStorage.setItem('serviceslocalemail_delMin', rates.minCharge)
        localStorage.setItem('serviceslocalemail_delMinKm', rates.minKm)
        localStorage.setItem('serviceslocalemail_delPerKm', rates.perKm)
        localStorage.setItem('serviceslocalemail_delMax', rates.maxKm)
        localStorage.setItem('serviceslocalemail_delCurrency', rates.currency)
        localStorage.setItem('serviceslocalemail_delRatesSet', 'true')
      }
    }
    const id = urlVendor || localStorage.getItem('serviceslocalemail_vendorId') || localStorage.getItem('serviceslocalemail_vendor_id')
    return !!id
  })
  ...
  const [shopTheme, setShopTheme] = useState(() => urlThemePreset ? urlThemePreset.id : (isDemo || isPreview) ? 'noodle' : (localStorage.getItem('serviceslocalemail_theme') || 'default'))
  const [shopAccentColor, setShopAccentColor] = useState(() => urlThemePreset ? urlThemePreset.accent : (isDemo || isPreview) ? '#8B0000' : (localStorage.getItem('serviceslocalemail_accentColor') || '#8DC63F'))
  ...
  const savedBgPos = (() => { try { return JSON.parse(localStorage.getItem('serviceslocalemail_bgPos')) } catch { return null } })()
  const bgStyle = shopTheme === 'custom' && savedBgPos ? { objectFit: 'cover', objectPosition: `${savedBgPos.x}% ${savedBgPos.y}%` } : { objectFit: 'fill' }
  const [delBaseFee, setDelBaseFee] = useState(() => parseInt(localStorage.getItem('serviceslocalemail_delBase')) || 5000)
  const [delPerKm, setDelPerKm] = useState(() => parseInt(localStorage.getItem('serviceslocalemail_delPerKm')) || 2500)
  const [delMinCharge, setDelMinCharge] = useState(() => parseInt(localStorage.getItem('serviceslocalemail_delMin')) || 7000)
  const [delMaxKm, setDelMaxKm] = useState(() => parseInt(localStorage.getItem('serviceslocalemail_delMax')) || 15)
  const [delFreeAbove, setDelFreeAbove] = useState(() => parseInt(localStorage.getItem('serviceslocalemail_delFree')) || 0)
  const [delCurrency, setDelCurrency] = useState(() => localStorage.getItem('serviceslocalemail_delCurrency') || 'Rp')
  const [delMinKm, setDelMinKm] = useState(() => parseInt(localStorage.getItem('serviceslocalemail_delMinKm')) || 2)
  const [delEnabled, setDelEnabled] = useState(() => localStorage.getItem('serviceslocalemail_delEnabled') !== 'false')

  /* Shop info */
  const [shopName, setShopName] = useState(() => localStorage.getItem('serviceslocalemail_shopName') || 'Your Store')
  const [shopLogo, setShopLogo] = useState(() => localStorage.getItem('serviceslocalemail_shopLogo') || 'https://fjvafjkzvygkhiwjuvla.supabase.co/storage/v1/object/public/assets/untitledsadaaaa-removebg-preview.png')
  const [shopLogoStyle, setShopLogoStyle] = useState(() => localStorage.getItem('serviceslocalemail_logoStyle') || 'circle') // circle | bare | off
  const [heroSize, setHeroSize] = useState(() => localStorage.getItem('serviceslocalemail_heroSize') || 'normal') // normal | large | xl
  const [heroFont, setHeroFont] = useState(() => localStorage.getItem('serviceslocalemail_heroFont') || 'system') // system | nunito | poppins | playfair | caveat | bebas
  const [heroColor, setHeroColor] = useState(() => localStorage.getItem('serviceslocalemail_heroColor') || '#ffffff')
  const [heroSubColor, setHeroSubColor] = useState(() => localStorage.getItem('serviceslocalemail_heroSubColor') || '') // empty = auto from heroColor
  const [heroEffect, setHeroEffect] = useState(() => localStorage.getItem('serviceslocalemail_heroEffect') || 'shadow') // shadow | glow | runGlow | outline | neon | none
  const [heroEditor, setHeroEditor] = useState(false) // full editor open
  const [shopPhone, setShopPhone] = useState(() => localStorage.getItem('serviceslocalemail_shopPhone') || '6281234567890')
  const [shopOpen, setShopOpen] = useState(() => loadJSON('serviceslocalemail_shopOpen', true))
  const [shopAddress, setShopAddress] = useState(() => localStorage.getItem('serviceslocalemail_shopAddress') || 'Jl. Malioboro, Yogyakarta')
  const [shopHours, setShopHours] = useState(() => localStorage.getItem('serviceslocalemail_shopHours') || '17:00 – 23:00')
  ...
  const [shopSchedule, setShopSchedule] = useState(() => loadJSON('serviceslocalemail_shopSchedule', defaultSchedule))
  const [shopMapsLink, setShopMapsLink] = useState(() => localStorage.getItem('serviceslocalemail_shopMaps') || '')
  const [shopInstagram, setShopInstagram] = useState(() => localStorage.getItem('serviceslocalemail_shopIG') || 'lummeenoodles')
  const [shopTiktok, setShopTiktok] = useState(() => localStorage.getItem('serviceslocalemail_shopTT') || 'lummeenoodles')
  const [shopFacebook, setShopFacebook] = useState(() => localStorage.getItem('serviceslocalemail_shopFB') || 'lummeenoodles')
  const [shopYoutube, setShopYoutube] = useState(() => localStorage.getItem('serviceslocalemail_shopYT') || 'lummeenoodles')
  const [shopWebsite, setShopWebsite] = useState(() => localStorage.getItem('serviceslocalemail_shopWeb') || 'www.lummeenoodles.com')
  const [shopQris, setShopQris] = useState(() => isDemo ? 'https://fjvafjkzvygkhiwjuvla.supabase.co/storage/v1/object/public/assets/untitledxzxcczdsasdsadads.png' : (localStorage.getItem('serviceslocalemail_shopQris') || 'https://fjvafjkzvygkhiwjuvla.supabase.co/storage/v1/object/public/assets/untitledxzxcczdsasdsadads.png'))
  const [shopBio, setShopBio] = useState(() => localStorage.getItem('serviceslocalemail_shopBio') || '')
  const [shopCity, setShopCity] = useState(() => localStorage.getItem('serviceslocalemail_shopCity') || '')
  const [shopCountry, setShopCountry] = useState(() => localStorage.getItem('serviceslocalemail_shopCountry') || '')
  const [shopFoodType, setShopFoodType] = useState(() => localStorage.getItem('serviceslocalemail_shopFoodType') || 'General Store')

  /* ─── Customization Features (all optional) ─── */
  const [btnShape, setBtnShape] = useState(() => localStorage.getItem('serviceslocalemail_btnShape') || 'rounded')
  const [btnColor, setBtnColor] = useState(() => localStorage.getItem('serviceslocalemail_btnColor') || '')
  const [btnText, setBtnText] = useState(() => localStorage.getItem('serviceslocalemail_btnText') || '')
  const [btnGlow, setBtnGlow] = useState(() => localStorage.getItem('serviceslocalemail_btnGlow') === 'true')
  const [overlayOpacity, setOverlayOpacity] = useState(() => parseInt(localStorage.getItem('serviceslocalemail_overlayOpacity')) || 40)
  const [landingLayout, setLandingLayout] = useState(() => localStorage.getItem('serviceslocalemail_landingLayout') || 'center')
  const [customTagline, setCustomTagline] = useState(() => localStorage.getItem('serviceslocalemail_customTagline') || '')
  const [menuCardStyle, setMenuCardStyle] = useState(() => localStorage.getItem('serviceslocalemail_menuCardStyle') || 'horizontal')
  const [menuBanner, setMenuBanner] = useState(() => localStorage.getItem('serviceslocalemail_menuBanner') || '')
  const [showClosedBanner, setShowClosedBanner] = useState(() => localStorage.getItem('serviceslocalemail_showClosedBanner') === 'true')
  const [promoBanner, setPromoBanner] = useState(() => localStorage.getItem('serviceslocalemail_promoBanner') || '')
  const [promoBannerEnabled, setPromoBannerEnabled] = useState(() => localStorage.getItem('serviceslocalemail_promoBannerEnabled') === 'true')
  const [splashEnabled, setSplashEnabled] = useState(() => localStorage.getItem('serviceslocalemail_splashEnabled') === 'true')
  const [showSplash, setShowSplash] = useState(() => localStorage.getItem('serviceslocalemail_splashEnabled') === 'true')
  ...
  const [dailyDeals, setDailyDeals] = useState(() => loadJSON('serviceslocalemail_dailyDeals', []))
  const [showCustomers, setShowCustomers] = useState(false)
  const [installDismissed, setInstallDismissed] = useState(() => localStorage.getItem('serviceslocalemail_installDismissed') === 'true')
  ...
  const [vendorOrderEmail, setVendorOrderEmail] = useState(() => localStorage.getItem('serviceslocalemail_orderEmail') || '')
  ...
  const [vendorId, setVendorId] = useState(() => new URLSearchParams(window.location.search).get('vendor') || localStorage.getItem('serviceslocalemail_vendorId') || localStorage.getItem('serviceslocalemail_vendor_id') || null)
  ...
  const [formCategory, setFormCategory] = useState(() => getMenuTabsForTheme(localStorage.getItem('serviceslocalemail_theme') || 'default')[0])
  ...
  useEffect(() => { if (vendorId) localStorage.setItem('serviceslocalemail_vendorId', vendorId) }, [vendorId])
  useEffect(() => { saveJSON('serviceslocalemail_menu', menuItems) }, [menuItems])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopName', shopName) }, [shopName])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopLogo', shopLogo) }, [shopLogo])
  useEffect(() => { localStorage.setItem('serviceslocalemail_logoStyle', shopLogoStyle) }, [shopLogoStyle])
  useEffect(() => { localStorage.setItem('serviceslocalemail_heroSize', heroSize) }, [heroSize])
  useEffect(() => { localStorage.setItem('serviceslocalemail_heroFont', heroFont) }, [heroFont])
  useEffect(() => {
    if (heroFont !== 'system') {
  ...
  useEffect(() => { localStorage.setItem('serviceslocalemail_heroColor', heroColor) }, [heroColor])
  useEffect(() => { localStorage.setItem('serviceslocalemail_heroSubColor', heroSubColor) }, [heroSubColor])
  useEffect(() => { localStorage.setItem('serviceslocalemail_heroEffect', heroEffect) }, [heroEffect])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopPhone', shopPhone) }, [shopPhone])
  useEffect(() => { saveJSON('serviceslocalemail_shopOpen', shopOpen) }, [shopOpen])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopAddress', shopAddress) }, [shopAddress])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopHours', shopHours) }, [shopHours])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopSchedule', JSON.stringify(shopSchedule)) }, [shopSchedule])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopMaps', shopMapsLink) }, [shopMapsLink])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopIG', shopInstagram) }, [shopInstagram])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopTT', shopTiktok) }, [shopTiktok])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopFB', shopFacebook) }, [shopFacebook])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopYT', shopYoutube) }, [shopYoutube])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopWeb', shopWebsite) }, [shopWebsite])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopQris', shopQris) }, [shopQris])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopFoodType', shopFoodType) }, [shopFoodType])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopBio', shopBio) }, [shopBio])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopCity', shopCity) }, [shopCity])
  useEffect(() => { localStorage.setItem('serviceslocalemail_shopCountry', shopCountry) }, [shopCountry])
  useEffect(() => { localStorage.setItem('serviceslocalemail_delBase', delBaseFee) }, [delBaseFee])
  useEffect(() => { localStorage.setItem('serviceslocalemail_delPerKm', delPerKm) }, [delPerKm])
  useEffect(() => { localStorage.setItem('serviceslocalemail_delMin', delMinCharge) }, [delMinCharge])
  useEffect(() => { localStorage.setItem('serviceslocalemail_delMax', delMaxKm) }, [delMaxKm])
  useEffect(() => { localStorage.setItem('serviceslocalemail_delFree', delFreeAbove) }, [delFreeAbove])
  useEffect(() => { localStorage.setItem('serviceslocalemail_delCurrency', delCurrency) }, [delCurrency])
  useEffect(() => { localStorage.setItem('serviceslocalemail_delMinKm', delMinKm) }, [delMinKm])
  useEffect(() => { localStorage.setItem('serviceslocalemail_delEnabled', delEnabled) }, [delEnabled])

  /* Customization features persistence */
  useEffect(() => { localStorage.setItem('serviceslocalemail_btnShape', btnShape) }, [btnShape])
  useEffect(() => { localStorage.setItem('serviceslocalemail_btnColor', btnColor) }, [btnColor])
  useEffect(() => { localStorage.setItem('serviceslocalemail_btnText', btnText) }, [btnText])
  useEffect(() => { localStorage.setItem('serviceslocalemail_btnGlow', btnGlow) }, [btnGlow])
  useEffect(() => { localStorage.setItem('serviceslocalemail_overlayOpacity', overlayOpacity) }, [overlayOpacity])
  useEffect(() => { localStorage.setItem('serviceslocalemail_landingLayout', landingLayout) }, [landingLayout])
  useEffect(() => { localStorage.setItem('serviceslocalemail_customTagline', customTagline) }, [customTagline])
  useEffect(() => { localStorage.setItem('serviceslocalemail_menuCardStyle', menuCardStyle) }, [menuCardStyle])
  useEffect(() => { localStorage.setItem('serviceslocalemail_menuBanner', menuBanner) }, [menuBanner])
  useEffect(() => { localStorage.setItem('serviceslocalemail_showClosedBanner', showClosedBanner) }, [showClosedBanner])
  useEffect(() => { localStorage.setItem('serviceslocalemail_promoBanner', promoBanner) }, [promoBanner])
  useEffect(() => { localStorage.setItem('serviceslocalemail_promoBannerEnabled', promoBannerEnabled) }, [promoBannerEnabled])
  useEffect(() => { localStorage.setItem('serviceslocalemail_splashEnabled', splashEnabled) }, [splashEnabled])
  ...
  useEffect(() => {
    ...
    supabase.from('vendor_accounts').select('*').eq('id', vendorId).single().then(({ data }) => {
      ...
      if (data.order_email) {
        ...
        try { localStorage.setItem('serviceslocalemail_orderEmail', data.order_email) } catch {}
      }
    })
  }, [vendorId])
  ...
  useEffect(() => {
    try { localStorage.setItem('serviceslocalemail_orderEmail', vendorOrderEmail || '') } catch {}
  ...
  const handleVendorLogin = async () => {
    ...
    if (vendor) {
      ...
      localStorage.setItem('serviceslocalemail_vendor_phone', loginPhone.replace(/[^0-9]/g, ''))
      localStorage.setItem('serviceslocalemail_vendor_pass', loginPass)
      setIsVendor(true); setVendorLogin(false)
      setLoginPhone(''); setLoginPass(''); setLoginError(''); setLoginMode('login')
      return
    }
    // Fallback to localStorage
    const storedPhone = localStorage.getItem('serviceslocalemail_vendor_phone') || shopPhone
    const storedPass = localStorage.getItem('serviceslocalemail_vendor_pass') || 'vendor123'
  ...
  const handleVendorSignup = async () => {
    ...
    try {
      ...
      localStorage.setItem('serviceslocalemail_shopFoodType', signupCategory)
      ...
      if (matchedTheme) {
        ...
        localStorage.setItem('serviceslocalemail_theme', matchedTheme.id)
        localStorage.setItem('serviceslocalemail_themeBg', matchedTheme.img)
        const bgImg = document.getElementById('app-bg-img')
  ...
  /* --- Email order (serviceslocalemail) --- */
  const sendOrderEmail = async () => {
    ...
    const customers = loadJSON('serviceslocalemail_customers', [])
    ...
    saveJSON('serviceslocalemail_customers', customers)
    if (supabase && vendorId && !String(vendorId).startsWith('local')) {
      try {
  ...
  if (showLanding) {
    return (
      <div style={{ width: '100%', height: '100%', overflow: 'hidden', position: 'relative' }}><DevBadge n={1} />
        ...
        <img src={localStorage.getItem('serviceslocalemail_themeBg') || 'https://fjvafjkzvygkhiwjuvla.supabase.co/storage/v1/object/public/assets/chatgpt-image-may-6-2026-01_19_01-pm.png'} alt="" onError={imgError('theme')} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', ...bgStyle }} />
    ...
    <div style={S.page}><DevBadge n={2} />
        ...
        {menuDrawerOpen && (
          <>
            ...
            <div role="dialog" aria-label="Browse services" style={{
              ...
              {[{ filter: 'All', label: 'All Services' }, ...MENU_CATEGORIES.slice(1).map(c => ({ filter: c, label: c }))].map(opt => {
      ...
      {itemModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 200 }} onClick={() => setItemModal(null)}><DevBadge n={3} />
          ...
          <img src={localStorage.getItem('serviceslocalemail_themeBg') || 'https://fjvafjkzvygkhiwjuvla.supabase.co/storage/v1/object/public/assets/chatgpt-image-may-6-2026-01_19_01-pm.png'} alt="" onError={imgError('theme')} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'fill', zIndex: 0 }} />
      ...
      {showLocation && (() => {
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 250 }}><DevBadge n={4} />
          <img src={localStorage.getItem('serviceslocalemail_themeBg') || 'https://fjvafjkzvygkhiwjuvla.supabase.co/storage/v1/object/public/assets/chatgpt-image-may-6-2026-01_19_01-pm.png'} alt="" onError={imgError('theme')} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'fill', zIndex: 0, pointerEvents: 'none' }} />
              ...
              <div style={{ position: 'relative' }}>
                ...
                <div style={{ width: 200, height: 400, borderRadius: 30, background: '#1a1a1a', padding: 3, position: 'relative', boxShadow: `0 12px 40px ${accent}20, 0 4px 16px rgba(0,0,0,0.3)`, border: '2px solid #333' }}>
                  ...
                  <div style={{ width: '100%', height: '100%', borderRadius: 27, overflow: 'hidden', position: 'relative', background: '#000' }}>
                    ...
                    {/* Background image */}
                    <img src={localStorage.getItem('serviceslocalemail_themeBg') || ''} alt="" onError={imgError('theme')} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'fill' }} />
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 600, background: '#ffffff', display: 'flex', flexDirection: 'column' }}><DevBadge n={5} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', padding: '8px 16px 24px', gap: 12 }}>
            <button onClick={() => {
              ...
              localStorage.setItem('serviceslocalemail_theme', 'custom'); localStorage.setItem('serviceslocalemail_themeBg', themeEditor.url)
              localStorage.setItem('serviceslocalemail_accentColor', editorColor); localStorage.setItem('serviceslocalemail_bgPos', JSON.stringify(editorPos))
              const bgImg = document.getElementById('app-bg-img')
              ...
              const renderThemeCard = (theme) => (
                <div key={theme.id} style={{ flexShrink: 0, width: 160, position: 'relative' }}>
                  <button onClick={() => {
                    ...
                    localStorage.setItem('serviceslocalemail_theme', theme.id)
                    localStorage.setItem('serviceslocalemail_themeBg', theme.img)
                    localStorage.setItem('serviceslocalemail_accentColor', theme.accent || '#8DC63F')
                    const bgImg = document.getElementById('app-bg-img')
                  ...
                  <div onClick={(e) => { e.stopPropagation(); setThemeEditor({ url: theme.img }); setEditorColor(theme.accent || '#8DC63F'); setEditorBaseColor(theme.accent || '#8DC63F'); setShopTheme(theme.id); setShopAccentColor(theme.accent || '#8DC63F'); localStorage.setItem('serviceslocalemail_theme', theme.id); localStorage.setItem('serviceslocalemail_themeBg', theme.img); localStorage.setItem('serviceslocalemail_accentColor', theme.accent || '#8DC63F'); const bgImg = document.getElementById('app-bg-img'); if (bgImg) bgImg.src = theme.img; setVendorDrawer(false) }} style={{ position: 'absolute', top: -6, right: -6, width: 30, height: 30, borderRadius: 15, background: '#FFD600', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.4)', zIndex: 2 }}>
      ...
      {showDeliverySettings && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 300 }}>
          <img src={localStorage.getItem('serviceslocalemail_themeBg') || 'https://fjvafjkzvygkhiwjuvla.supabase.co/storage/v1/object/public/assets/chatgpt-image-may-6-2026-01_19_01-pm.png'} alt="" onError={imgError('theme')} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', ...bgStyle, zIndex: 0 }} />
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', zIndex: 0 }} />
            ...
            <div style={{ textAlign: 'center', marginBottom: 16 }}>
              <label style={{ cursor: 'pointer', display: 'inline-block' }}>
                <input type="file" accept="image/*" style={{ display: 'none' }} onChange={async (e) => {
                  ...
                  if (!file) return
                  const url = await uploadMenuImage(vendorId, file)
                  if (url) { setShopLogo(url); localStorage.setItem('serviceslocalemail_shopLogo', url) }
                }} />
                {shopLogo ? (
              ...
              {shopLogo && <button onClick={() => { setShopLogo(''); localStorage.removeItem('serviceslocalemail_shopLogo') }} style={{ background: 'none', border: 'none', color: '#EF4444', fontSize: 12, fontWeight: 700, cursor: 'pointer', marginTop: 4 }}>Remove</button>}
            ...
            <div style={{ textAlign: 'center' }}>
              <label style={{ cursor: 'pointer', display: 'inline-block' }}>
                <input type="file" accept="image/*" style={{ display: 'none' }} onChange={async (e) => {
                  ...
                  if (!file) return
                  const url = await uploadMenuImage(vendorId, file)
                  if (url) { setShopQris(url); localStorage.setItem('serviceslocalemail_shopQris', url) }
                }} />
                {shopQris ? (
              ...
              {shopQris && (
                <div style={{ marginTop: 8 }}>
                  <button onClick={() => { setShopQris(''); localStorage.removeItem('serviceslocalemail_shopQris') }} style={{ background: 'none', border: 'none', color: '#EF4444', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Remove QR</button>
                </div>
              )}
            </div>
          </div>
                  ...
                  <div style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(0,0,0,0.92)', display: 'flex', flexDirection: 'column', alignItems: 'center', overflowY: 'scroll', WebkitOverflowScrolling: 'touch' }} onClick={() => { setThemePreviewId(null); setThemePreviewImg(null) }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, flexShrink: 0, paddingBottom: 20 }} onClick={e => e.stopPropagation()}>
                      ...
                      <button onClick={() => {
                        ...
                        localStorage.setItem('serviceslocalemail_theme', theme.id); localStorage.setItem('serviceslocalemail_themeBg', activeImg); localStorage.setItem('serviceslocalemail_accentColor', theme.accent || '#8DC63F')
      ...
      {designStudio && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200 }}><DevBadge n={11} />
          <img src={localStorage.getItem('serviceslocalemail_themeBg') || ''} alt="" onError={imgError('theme')} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', ...bgStyle, zIndex: 0 }} />
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', zIndex: 0 }} />
          <div style={{ position: 'relative', zIndex: 1, width: '100%', height: '100%', maxWidth: 480, margin: '0 auto', overflowY: 'auto' }}>
            {/* Header */}
            <div style={{ display: 'flex', alignItems: 'center', padding: '14px 16px', gap: 10 }}>
                              ...
                              <div style={{ display: 'flex', gap: 3 }}>
                                {shades.map((c, i) => (
                                  <button key={i} onClick={() => {
                                    ...
                                    localStorage.setItem('serviceslocalemail_accentColor', c)
                                  }} style={{
