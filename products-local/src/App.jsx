...
export default function App() {
  ...
  const [design, setDesign] = useState(() => {
    try {
      const raw = localStorage.getItem('productslocal_design')
  ...
  // Hydrate paymentGateways from the DB when the vendor logs in / changes.
  // This is what makes connections survive a fresh device or browser.
  useEffect(() => {
    ...
    <div style={S.page}><DevBadge n={2} />
      ...
      {isVendor && setupGatewayId && (() => {
        ...
        const save = async () => {
          ...
          // Persist to vendor_payment_connections so the connection survives
          // device changes and the customer-side charge fn can read it.
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 10005, background: '#0a0a0a', overflowY: 'auto' }}>
          <ChannelSettings
            ...
            onSave={async (channels) => {
              ...
              localStorage.setItem('productslocal_channels', JSON.stringify(channels))
              if (supabase && vendorId && !String(vendorId).startsWith('local')) {
      ...
      {designStudioOpen && (
        <DesignStudio
          ...
          vendor={{ shopName, shopLogo, shopType: shopFoodType, shopOpen }}
          bgUrl={localStorage.getItem('productslocal_themeBg') || ''}
          accent={accent}
          categories={(getCategoriesForTheme(shopTheme, 'products') || []).map((c) => ({ id: c.id, label: c.label }))}
          appKind="products"
          ...
          onUploadImage={async (file) => {
            ...
            try {
              ...
              const path = `products/banners/${vendorId || 'anon'}/${Date.now()}.${ext}`
      ...
      {/* ── Floating vendor controls (when logged in as vendor) ───── */}
