...
export default function App() {
  ...
  // central account. Webhook flips vendor.status to 'active'. product='services'
  // selects the right PRICE_TABLE row.
  const startSubscriptionCheckout = async (planTier) => {
    if (!supabase || !vendorId || subBusy) return
    setSubBusy(true); setSubError('')
    try {
      emitFunnelStep('payment_started', { vendorId, metadata: { plan_tier: planTier, product: 'services' } })
      const returnUrl = window.location.origin + window.location.pathname
      const { data, error } = await supabase.functions.invoke('subscription-create-checkout', {
        body: { vendorId, planTier, product: 'services', returnUrl },
      })
      ...
      try { localStorage.setItem('vendorservices_pendingSubOrder', JSON.stringify({ orderId: data.orderId, vendorId, planTier, at: Date.now() })) } catch {}
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (data?.status === 'active') {
          ...
          emitFunnelStep('payment_completed', { vendorId, metadata: { plan_tier: data.plan_tier || null, product: 'services' } })
          try { localStorage.removeItem('vendorservices_pendingSubOrder') } catch {}
          return
        }
      } catch {}
  ...
  useEffect(() => {
    ...
    supabase.from('vendor_accounts').select('*').eq('id', vendorId).single().then(({ data }) => {
      ...
      if (data.design_config && typeof data.design_config === 'object' && Object.keys(data.design_config).length > 0) {
        ...
        try { localStorage.setItem('vendorservices_design', JSON.stringify({ ...DEFAULT_DESIGN, ...data.design_config })) } catch {}
      }
    })
  }, [vendorId])
  ...
  const sendWhatsApp = async () => {
    ...
    // Gateway pre-check — if vendor has Midtrans/Stripe/Xendit connected and
    // the in-app chat channel is on, mint a vendor_orders row and open the
    // CheckoutSheet instead of WhatsApp/email.
      ...
      try {
        ...
        if (conn) {
          const { data: row, error } = await supabase.from('vendor_orders').insert({
            ...
            payment_status: 'pending', status: 'new', module: 'services',
            note: note || null,
          }).select().single()
          if (!error && row) {
    ...
    <div style={S.page}>
      ...
      {dashboardOpen && (
        <DashboardShell
          supabase={supabase}
          vendorId={vendorId}
          title="Services Local — Vendor Dashboard"
          accent="#DC2626"
          statusColumns={[
            { id: 'new',        label: 'New',        color: '#9CA3AF' },
            { id: 'confirmed',  label: 'Confirmed',  color: '#22C55E' },
            { id: 'scheduled',  label: 'Scheduled',  color: '#FACC15' },
      ...
      {isVendor && (
        <SideDrawer
          ...
          tiles={[
            ...
            { id: 'delivery',  icon: '🛵', label: 'Service Area',     desc: 'Coverage, zones, distance',    onClick: () => { setShowDeliverySettings(true); setVendorDrawerOpen(false) } },
